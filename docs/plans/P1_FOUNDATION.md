...
状态：进行中（前三批实现、本地验证和远端 CI 完成；第四批验收补齐进行中）
...
第四批当前证据：状态码 0～16 直接数值测试、COBS 容量不足测试、主要宽度/端序与浮点特殊值、
...
三批合计 11 个基础组件均具备公共接口、构建目标、组件说明及相应测试；
foundation_status 为轻量头文件契约，其状态码在其他组件测试中使用。
第一、第二批及第三批的本地和远端验证证据见上文；第四批完成后才将 P1 标记为已完成。
...
下一阶段为 P2 控制与算法库，仍由用户手动开启阶段性 Plan；第四批完成前不进入 P2。
