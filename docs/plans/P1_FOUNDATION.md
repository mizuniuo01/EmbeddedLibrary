...
状态：已完成（四批实现、契约补齐、本地验证和远端 CI 均已闭环）
...
第四批证据：状态码 0～16 直接数值测试、COBS 容量不足测试、主要宽度/端序与浮点特殊值、
CRC 分块等价性、四个容器的多实例基本隔离和外部 CMake 接入夹具已加入；`sh tools/check.sh host-gcc`
已通过，包含 12 项仓库测试和 1 项外部接入测试，并通过 clang-format、Clang-Tidy 和 Cppcheck。
多实例隔离、外部 CMake 接入和完整本地工具链验证均已通过。

远端证据：提交 `1099f6ab8e1df7a4393606634f1a0686d27dca` 对应 GitHub Actions 运行
[`34014692455`](https://github.com/mizuniuo01/EmbeddedLibrary/actions/runs/34014692455)，
`host (gcc)`、`host (clang)`、`sanitizer`、`cmake-minimum` 和 `arm-cortex-m0plus`
五项任务全部成功。

成熟度：`host-tested / hardware-unverified`，不是 `rtos-tested`。
远端证据：提交 `ceb58a9ffee3d2fe5c4a4091fd96783f429872c3` 对应 GitHub Actions 运行
[`33962386609`](https://github.com/mizuniuo01/EmbeddedLibrary/actions/runs/33962386609)，
`host (gcc)`、`host (clang)`、`sanitizer`、`cmake-minimum` 和 `arm-cortex-m0plus`
五项任务全部成功，其中最低 CMake 3.20.6 验证通过。第三批验收完成。

## 7. 第四批：契约补齐与复用验收

第四批不新增 P1 生产组件，补齐前三批的直接契约证据和文档一致性：锁定状态码数值，
覆盖所有大小端/宽度组合、浮点特殊值、CRC 分块、COBS 容量与确定性往返、失败输出保持、
多实例隔离和容器参考模型；增加组件最小外部工程接入验证。

本批完成条件：每个公开 API 有对应直接测试，组件文档只声明实际覆盖内容，独立接入夹具
可构建并运行，GCC/Clang/Sanitizer/CMake 最低版本/ARM CI 全部通过。SLIP、RTOS、硬件、
WCET 和 P2 算法不纳入本批。推荐单提交标题为 `test(p1): complete contract coverage and reuse validation`。

## 8. P1 收尾与后续入口

四批合计 11 个基础组件均具备公共接口、构建目标、组件说明及相应测试；
foundation_status 为轻量头文件契约，其状态码在其他组件测试中使用。
第一至第四批的本地和远端验证证据见上文，P1 已标记为已完成。

保留限制：本阶段没有验证 RTOS 调度、目标硬件行为或目标 WCET；容器依赖外部同步，
地址检查依赖有记录的平坦地址 ABI。对象池不识别槽地址重用后的旧指针，
快照每槽最多一个读借用，借用不归还会持续产生背压。这些是已记录的使用边界，
不能因 CI 成功而提升为 rtos-tested 或硬件验证完成。

下一阶段为 P2 控制与算法库，仍由用户手动开启阶段性 Plan。
规划范围为通用 PID、串级/组合控制器、一阶滤波、滑动平均、迟滞、死区和速率限制。
需在实现前明确时间步长、状态复位、参数修改、积分抗饱和、组合控制器职责及算法测试判据；
规范规定之外的设计细节继续与用户确认，本次不冻结 P2 API 或启动 P2 实现。
