...
已完成的 P0-P3 是现有资产，不等同于最终主实现；所有能力均可重新评估。未开始能力先进行成熟库调查，满足规范的成熟库通过仓库适配层作为主实现，只有架构核心或无合适成熟库时才自研。适配层不得泄漏第三方类型、线程模型、动态分配或平台依赖。默认采用源码 vendoring、版本锁定和许可证审计；现有公共 API 尽量保持不变。
...
快照借用生命周期限制见阶段与组件文档。第四批契约补齐、独立接入验证和远端验收已完成，
P1 全部四批闭环。

### P2：控制与算法库 — 已完成

实现通用 f32 PID、固定两级事务式串级 PID、一阶滤波、滑动平均、迟滞、死区和速率限制。
P2 三批均支持多实例、静态分配和主机测试，不包含航向、左右轮、滚球等项目语义。
第一批提交 `f890d0a3ffb12e90f065d60c0594d4b41cf244f6` 对应运行
`34022060895` 全部成功；第二批提交 `968b0588aeebd10e5f9a968ba9b3f10827e0dbe9` 对应运行
`34087853238` 全部成功；第三批提交 `3b9efb6e6714690712ed6f77b64ec8b28ef10d2a` 对应运行
[`34128638025`](https://github.com/mizuniuo01/EmbeddedLibrary/actions/runs/34128638025)
...
建立逐能力成熟库对比矩阵，覆盖 P0-P3 既有能力和所有未来能力；记录候选库、版本、许可证、资源、平台隔离、规范符合性、测试证据及保留/替换/适配结论。本阶段不新增生产功能。

### R1：架构核心运行时 — 未开始

自研协作式静态调度器、release/deadline、漏周期、工作配额、过载、任务健康和统一看门狗监督契约；底层时间、容器和同步能力优先复用已评估实现。

### R2：成熟库集成与适配基础 — 未开始

建立 vendoring、版本锁定、第三方隔离、适配层、主机替身和依赖审计机制。
...
优先复用或适配成熟日志、错误、诊断、参数和 Repository 能力；不实现具体 Flash/NVM、看门狗或 MCU 驱动。

### R5：菜单、显示和蓝牙 — 未开始

优先评估成熟菜单、显示和蓝牙协议库；显示不得依赖蓝牙；蓝牙只是一种可替换的传输/协议/诊断输出适配器。OLED 字模和具体屏幕 Driver 不纳入。
...
| 2026-09-13 | 改为成熟库优先并启动全仓库能力审查 | 避免重复实现已有成熟能力，保留架构核心自研 |
